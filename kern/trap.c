...
trap_init_percpu(void)
{
	...
	int curr_cpu_i, kstacktop_i;

	curr_cpu_i = cpunum();
	kstacktop_i = KSTACKTOP - curr_cpu_i * (KSTKSIZE + KSTKGAP);

	// Setup a TSS so that we get the right stack
	// when we trap to the kernel.
	cpus[curr_cpu_i].cpu_ts.ts_esp0 = kstacktop_i;
	cpus[curr_cpu_i].cpu_ts.ts_ss0 = GD_KD;

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + curr_cpu_i] = SEG16(STS_T32A, (uint32_t)(&(cpus[curr_cpu_i].cpu_ts)), sizeof(struct Taskstate) - 1, 0);
	gdt[(GD_TSS0 >> 3) + curr_cpu_i].sd_s = 0;

	// Load the TSS selector (like other segment selectors, the
	// bottom three bits are special; we leave them 0)
	ltr(GD_TSS0 + 8 * curr_cpu_i);
